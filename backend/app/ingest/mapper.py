"""对方科目规则预填服务（决策 P1：规则命中自动预填 + 其余批量人工指定）。

借鉴 bankstatementparser.enrichment.account_mapper 的有序规则、首条命中设计：
- 匹配目标 = 摘要(summary) + 对方户名(counterparty_name) 归一化后的文本；
- 规则按 priority 升序、同优先级按 mapping_id 排，首条命中即取用；
- 命中结果以命名 key 写入 flow.ext_json，供复核工作台展示"自动预填科目"；
- 未命中则保持空，流水流入"批量人工指定"队列。
"""
