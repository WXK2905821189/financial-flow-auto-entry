"""启动播种：维度字典 + 校验规则种子 + 初始管理员（幂等）。

生产由运维用 financial_flow_schema.sql 建库；本模块在开发期做等价播种，
保证「启动即可跑通端到端 Mock 演示」。
"""
...
def _seed_rules(db: Session) -> None:
    rules = [
        ...
        ("R003", "必填字段缺失", "ERROR", "契约核心字段（日期/金额/方向/流水号）缺失"),
        ("R003b", "对方户名缺失", "WARN", "对方户名缺失（财务 P3 降级，转人工复核不阻塞）"),
...
def _seed_mappings(db: Session) -> None:
    """科目规则示例种子（决策 P1 规则命中的演示口径）。

    生产规则表由财务在 UAT 前维护（P4 行动项：与银行存款科目映射表一并提供）。
    关键词方向：CREDIT(收入)/DEBIT(支出)/BOTH；优先级升序，首条命中即取。
    """
    seeds = [
        # (pattern, match_type, direction, subject_code, subject_name, priority)
