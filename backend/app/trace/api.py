...
def by_voucher(
    ...
    user: dict = Depends(require_permission(Permission.TRACE_READ.value)),
):
    ...
    return _record_trace(db, push.record_id, user, request)
...
def by_record(
    ...
    return _record_trace(db, record_id, user, request)


def _record_trace(db: Session, record_id: int, user: dict, request: Request | None = None) -> dict:
    ...
    batch_validations = db.execute(
        select(FlowValidation).where(
            FlowValidation.batch_id == flow.batch_id,
    ...
    return {
        "flow": {
            ...
            "counterparty_name": flow.counterparty_name if allow_pii else mask_name(flow.counterparty_name),
            "counterparty_account": flow.counterparty_account if allow_pii else mask_account(flow.counterparty_account),
        ...
        "account": {"account_no": account.account_no if allow_pii else mask_account(account.account_no), "account_name": account.account_name if allow_pii else mask_name(account.account_name)} if account else None,
        "raw": {"raw_id": raw.raw_id, "raw_hash": raw.raw_hash, "raw_content": raw.raw_content if allow_pii else None} if raw else None,
        "validations": [
