...
class FlowBatch(Base):
    ...
    # 批次统计回流（对接 v_recon_balance 对账口径）：loaded + duplicated = total_count
    ...
    # 批次余额勾稽（规则 R006）：来源银行报表头部的期初/期末余额；无可勾稽即 SKIP
    expected_begin_balance: Mapped[Decimal | None] = mapped_column(DECIMAL(20, 2), nullable=True)
    expected_end_balance: Mapped[Decimal | None] = mapped_column(DECIMAL(20, 2), nullable=True)
    balance_check_status: Mapped[str] = mapped_column(String(8), nullable=False, default="SKIP")  # PASS / FAIL / SKIP
    balance_diff: Mapped[Decimal | None] = mapped_column(DECIMAL(20, 2), nullable=True)
