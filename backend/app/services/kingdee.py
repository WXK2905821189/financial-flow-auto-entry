...
_COUNTERPARTY_SUBJECT_NO = "2202"  # 往来/对方科目占位（按流水性质决定，实际需分类映射）
_BOOK_NO = "001"                  # 账簿编码占位
...
class KingdeeClient:
    ...
    def push_voucher(self, flow: TransFlow) -> dict:
...
class MockKingdeeClient(KingdeeClient):
    ...
    def push_voucher(self, flow: TransFlow) -> dict:
        ...
        return {"voucher_no": voucher_no, "doc_no": f"DOC-{flow.txn_no}", "mock": True}
...
def build_voucher_model(flow: TransFlow) -> dict[str, Any]:
    ...
    金额以字符串传递以保留 Decimal 精度；科目/账簿/币别见模块级 TODO 占位。
    """
    ...
    return {
        ...
        "FEntity": [
            ...
            _entry(_COUNTERPARTY_SUBJECT_NO, counterparty_side),
        ],
    }
