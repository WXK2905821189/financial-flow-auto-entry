...
## 五、落地状态（2026-08-25 · 已实现）

产品经理对话已亲自落地「R006 批次勾稽 + 科目规则预填」两项行动项，代码位于 `backend/app/`：

### 已落地的改动
| 文件 | 改动 |
|---|---|
| `models/dim.py` | 新增 `AccountMapping` 科目规则表（pattern/match_type/direction/subject_code/priority） |
| `models/dwd.py` | `FlowBatch` 新增期初/期末余额 + `balance_check_status` + `balance_diff` 字段 |
| `services/validation.py` | 新增 `check_batch_balance()`（R006，Golden Rule 勾稽，容差 0.01） |
| `services/account_mapper.py` | **新增**：科目规则匹配服务（首条命中、关键词/正则、方向过滤） |
| `services/ingest.py` | 支持传入期初/期末余额并执行批次勾稽；落库每笔后规则预填科目到 `ext_json.auto_subject` |
| `services/seed.py` | 新增 R006 规则字典 + 6 条科目映射演示种子（生产表由财务 UAT 前提供） |
| `api/ingest.py` | mock/file 接口支持 `begin_balance`/`end_balance` 参数 |
| `api/review.py` | 复核列表透出 `auto_subject`（供复核台展示规则预填科目） |

### 验证结果
- `smoke_test.py`：原有 21 项全绿，无回归。
- `verify_r006_mapping.py`（专项，新增）：R006-PASS / R006-FAIL / R006-SKIP / 科目命中 / 未命中 / 方向过滤 **6 项全通过**。

### 遗留提示
- R006 依赖银行报表带期初/期末余额；字段能否取得，需 8/27 契约冻结前财务业务顾问确认（当前为**可选启用**，无余额默认 SKIP 不阻塞）。
- `AccountMapping` 生产规则由财务 UAT 前维护（P4 行动项之一），现有种子仅为演示口径。