...
async function renderAlert(root) {
  ...
  const listRows = rows.map((r) => {
    ...
    return `<tr ${batchLevel ? `data-batch="${r.batch_id}"` : `data-id="${r.record_id}"`}>
      ...
      <td>${batchLevel ? '<button class="link-btn batch-open" data-batch="' + r.batch_id + '">去对账</button>' : `<button class="link-btn row-open" data-id="${r.record_id}">去复核</button>`}</td>
    </tr>`;
  }).join("");
  ...
  root.innerHTML = `
    <div class="section-gap">
      <div class="stat-row">
        ...
        ${statCard("覆盖规则类型", `<span style="color:var(--info)">${ui.fmtInt(ruleSet.size)}</span>`, "R001–R006")}
      </div>
      <div class="grid-2">
        ...
        ${wrapCard("规则说明", `<div class="muted" style="line-height:2;font-size:13px">
          ...
          <div>· <b>批次余额勾稽</b>：期初 + 收入 − 支出须等于期末，否则整批告警（R006）。</div>
  ...
  root.querySelectorAll("tr[data-batch]").forEach((tr) => {
    tr.addEventListener("click", () => {
      document.querySelector('.nav-item[data-view="dash"][data-tab="recon"]')?.click();
    });
  });
  root.querySelectorAll(".batch-open").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      document.querySelector('.nav-item[data-view="dash"][data-tab="recon"]')?.click();
    });
