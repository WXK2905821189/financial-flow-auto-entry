...
export function formModal({ title, sub = "", html, okText = "确认", danger = false, onOk }) {
  const okCls = danger ? "btn-danger" : "btn-pri";
  openModal({
    icon: danger ? "warn" : "ok",
    title,
...
export function messageModal({ title, sub = "", html, okText = "关闭", icon = "ok" }) {
  openModal({
    icon,
    title,
    sub,
    body: html || "",
    actions: `<button class="btn btn-pri" data-ok>${esc(okText)}</button>`,
  });
  document.getElementById("mo-ft").querySelector("[data-ok]").onclick = closeModal;
}
