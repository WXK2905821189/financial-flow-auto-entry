...
const RULE_LABEL = {
  ...
  R004: "超阈值", R005: "币种异常", R006: "批次余额勾稽",
};
...
function renderTrace(tr) {
  ...
  const validations = [
    ...(tr.validations || []),
    ...(tr.batch_validations || []).map((item) => ({ ...item, batch_level: true })),
  ];
  let vHtml;
  if (validations.length === 0) {
    ...
    vHtml = validations.map((v) => {
      ...
      const [label, color] = map[v.rule_result] || ["—", "var(--muted)"];
       const rule = `${RULE_LABEL[v.rule_code] ? `${v.rule_code} · ${RULE_LABEL[v.rule_code]}` : v.rule_code}${v.batch_level ? " · 批次级" : ""}`;
...
async function doPushOnce(record_id, onChanged) {
  ...
  if (p.push_status !== "SUCCESS") throw new Error(p.error_msg || "金蝶推送失败");
