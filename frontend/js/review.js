// 复核工作台：统计卡 + 流水队列 + 筛选 + 批量操作 + Mock/文件采集入口
    ...
    <div class="section-gap">
      ...
      <div class="card">
        <div class="card-hd">
          ...
          <div class="right">
            <button class="btn btn-ghost btn-sm" id="btn-ingest">采集流水</button>
...
function bindEvents() {
  document.getElementById("chk-all").addEventListener("change", (e) => {
    selected = new Set();
    if (e.target.checked) filteredRows().filter((r) => r.process_status === "REVIEW_READY").forEach((r) => selected.add(r.record_id));
    renderRows();
    refreshBatchState();
  });
...
function renderRows() {
  ...
  if (rows.length === 0) {
    tbody.innerHTML = `<tr><td colspan="11">${ui.emptyHtml("暂无符合条件的流水")}</td></tr>`;
    return;
  }
  tbody.innerHTML = rows.map((r) => `
    <tr data-id="${r.record_id}" class="${selected.has(r.record_id) ? "checked" : ""}">
      <td>${r.process_status === "REVIEW_READY" ? `<label class="ck"><input type="checkbox" class="row-chk" data-id="${r.record_id}" ${selected.has(r.record_id) ? "checked" : ""} /></label>` : ""}</td>
...
function refreshBatchState() {
  ...
  document.getElementById("btn-push").disabled = n === 0;
}
...
async function batchDecide(result) {
  ...
  ui.confirmModal({
    ...
    okText: "确认通过",
    onOk: async () => {
      const result = await api.decideBatch(ids, "PASS");
      const skipped = (result.items || []).filter((item) => item.result !== "PASS");
      if (!skipped.length) {
        ui.toast(`已通过 ${ids.length} 笔`, "success");
      } else {
        const detail = skipped.map((item) => `<div class="trace-item"><span class="t">流水 #${item.record_id}</span><span class="c">${ui.esc(item.error || "未完成复核")}</span></div>`).join("");
        setTimeout(() => ui.messageModal({
          title: "批量复核完成",
          sub: `通过 ${ids.length - skipped.length} 笔，未通过 ${skipped.length} 笔`,
          html: detail,
          icon: "warn",
        }), 0);
      }
...
async function pushSelected() {
  ...
  ui.confirmModal({
    ...
    onOk: async () => {
      const reviewResult = await api.decideBatch(ids, "PASS");
      const succeeded = [];
      const failed = (reviewResult.items || [])
        .filter((item) => item.result !== "PASS")
        .map((item) => ({ id: item.record_id, reason: item.error || "未完成复核" }));
      const reviewedIds = new Set((reviewResult.items || [])
        .filter((item) => item.result === "PASS")
        .map((item) => item.record_id));
      for (const id of ids.filter((item) => reviewedIds.has(item))) {
        try {
          const result = await api.pushRecord(id);
          if (result.push_status !== "SUCCESS") throw new Error(result.error_msg || "金蝶返回失败");
          succeeded.push({ id, voucher: result.voucher_no });
        } catch (e) {
          failed.push({ id, reason: e.message || "推送失败" });
        }
      }
      await reload();
      if (!failed.length) {
        ui.toast(`已推送 ${succeeded.length} 笔至金蝶 · 凭证已生成`, "success");
        return;
      }
      const detail = failed.map((item) => `<div class="trace-item"><span class="t">流水 #${item.id}</span><span class="c">${ui.esc(item.reason)}</span></div>`).join("");
      setTimeout(() => ui.messageModal({
        title: "批量推送完成",
        sub: `成功 ${succeeded.length} 笔，失败 ${failed.length} 笔，失败记录仍可在详情中重试`,
        html: detail,
        icon: "warn",
      }), 0);
    },
...
function openIngestModal() {
  ui.formModal({
    title: "采集流水",
    sub: "导入后自动标准化、校验并进入复核队列",
    okText: "开始采集",
    html: `
      <div class="form-grid">
        <label class="form-field">采集方式
          <select class="select" data-ingest-mode>
            <option value="MOCK">Mock 流水</option>
            <option value="FILE">上传文件</option>
          </select>
        </label>
        <label class="form-field">银行
          <select class="select" data-ingest-bank>
            <option value="CITIC">中信银行（CITIC）</option>
            <option value="CMB">招商银行（CMB）</option>
          </select>
        </label>
        <label class="form-field">账户号
          <input class="input" data-ingest-account value="1100000000001" placeholder="本方银行账户号" />
        </label>
        <label class="form-field" data-ingest-count-field>Mock 笔数
          <input class="input" data-ingest-count type="number" min="1" max="500" value="50" />
        </label>
        <label class="form-field" data-ingest-file-field style="display:none">流水文件
          <input class="input" data-ingest-file type="file" accept=".csv,.xlsx,.xls" />
        </label>
        <label class="form-field">期初余额（可选）
          <input class="input" data-ingest-begin type="number" min="0" step="0.01" placeholder="填写后启用 R006" />
        </label>
        <label class="form-field">期末余额（可选）
          <input class="input" data-ingest-end type="number" min="0" step="0.01" placeholder="填写后启用 R006" />
        </label>
      </div>`,
    onOk: async () => {
      const body = document.getElementById("mo-body");
      const mode = body.querySelector("[data-ingest-mode]").value;
      const bank = body.querySelector("[data-ingest-bank]").value;
      const account = body.querySelector("[data-ingest-account]").value.trim();
      const begin = body.querySelector("[data-ingest-begin]").value.trim();
      const end = body.querySelector("[data-ingest-end]").value.trim();
      if (!account) throw new Error("账户号不能为空");
      if ((begin && !end) || (!begin && end)) throw new Error("期初和期末余额需同时填写");
      let result;
      if (mode === "FILE") {
        const file = body.querySelector("[data-ingest-file]").files[0];
        if (!file) throw new Error("请选择流水文件");
        result = await api.ingestFile(file, bank, account, begin, end);
      } else {
        const count = Number(body.querySelector("[data-ingest-count]").value);
        if (!Number.isInteger(count) || count < 1 || count > 500) throw new Error("Mock 笔数需为 1 到 500 的整数");
        result = await api.ingestMock(bank, account, count, begin, end);
      }
      await reload();
      const batchText = (result.batches || []).map((b) => b.batch_no).join("、");
      const tone = result.failed ? "error" : result.warned ? "info" : "success";
      ui.toast(`采集完成：入库 ${result.loaded} 笔，重复 ${result.duplicated} 笔，异常 ${result.failed} 笔，告警 ${result.warned} 笔${batchText ? ` · ${batchText}` : ""}`, tone, 5000);
    },
  });
  const body = document.getElementById("mo-body");
  const mode = body.querySelector("[data-ingest-mode]");
  const syncMode = () => {
    const file = mode.value === "FILE";
    body.querySelector("[data-ingest-file-field]").style.display = file ? "flex" : "none";
    body.querySelector("[data-ingest-count-field]").style.display = file ? "none" : "flex";
  };
